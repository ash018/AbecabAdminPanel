...
/* Access Controll Related Table*/
CREATE TABLE Role(
	`RoleId` int(11) primary key auto_increment ,
	`RoleName` varchar(200) unique not  null,
	`EntryBy` varchar(250) default 'admin',
	`EntryDate` datetime default now(),
	`EditedBy` varchar(250) default 'admin',
	`EditedDate` datetime default now()
)ENGINE=InnoDB;

CREATE TABLE AccessUrl(
	`AccessUrlId` int(11) primary key auto_increment ,
	`AccessUrlName` varchar(200) unique not  null
)ENGINE=InnoDB;

CREATE TABLE RoleAccess(
	`RoleAccessId` int(11) primary key AUTO_INCREMENT,
	`RoleId` int(11) not null,
    `AccessUrlId` int(11) not null,
     FOREIGN KEY fk_role_id(RoleId)
	 REFERENCES Role(RoleId) 
	 ON DELETE cascade,
     FOREIGN KEY fk_access_url_id(AccessUrlId)
	 REFERENCES AccessUrl(AccessUrlId) 
	 ON DELETE cascade
    )ENGINE=InnoDB;
/* Add Role To User_Manager table*/

 SET FOREIGN_KEY_CHECKS=0;
    alter table usermanager 
    add column RoleId int(11) default 0,
    add FOREIGN KEY fk_role_id(RoleId)
	REFERENCES role(RoleId) 
	on delete no action;
